...
### 3.2 Pipeline 全流程模式（含实验执行）

先填写任务文件（参考第 11 节），随后在终端运行。
...
powershell -ExecutionPolicy Bypass `
  -File research_pipeline/scripts/run-pipeline.ps1 `
  -TaskFile research_pipeline/tasks/my-task.json -AutoConfirm -StrictExec

# 从上次中断续跑（跳过已有产物的阶段）
powershell -ExecutionPolicy Bypass `
  -File research_pipeline/scripts/run-pipeline.ps1 `
  -TaskFile research_pipeline/tasks/my-task.json -Resume -AutoConfirm -StrictExec

# 验证结构而不执行实验命令
powershell -ExecutionPolicy Bypass `
  -File research_pipeline/scripts/run-pipeline.ps1 `
  -TaskFile research_pipeline/tasks/my-task.json -DryRun -AutoConfirm -StrictExec
```
...
# 从上次中断续跑
bash research_pipeline/scripts/run-pipeline.sh \
  --task research_pipeline/tasks/my-task.json --resume --auto-confirm --strict-exec

# 验证结构而不执行实验命令
bash research_pipeline/scripts/run-pipeline.sh \
  --task research_pipeline/tasks/my-task.json --dry-run --auto-confirm --strict-exec
```

> **依赖**：Bash 脚本需要 `jq`（JSON 处理）和 `bc`（浮点运算）。Ubuntu 安装：`sudo apt-get install jq bc`

`-AutoConfirm` / `--auto-confirm` 启用无人值守推进：跳过人工确认提示，自动继续后续阶段。
`-StrictExec` / `--strict-exec` 启用严格执行：缺输入/缺输出/缺命令或命令失败时立即终止流程。

编码建议：任务文件与产物状态文件请优先使用 UTF-8。当前 pipeline 已增加 JSON 多编码容错读取，但保持 UTF-8 仍是最稳妥配置。
...
# 全量迭代
powershell -ExecutionPolicy Bypass `
  -File research_pipeline/scripts/run-iterative.ps1 `
  -RunDir research_runs/tabular-foundation-multimodal-causal/20260331_run01 `
  -Mode full -AutoConfirm

# 仅补跑实验
powershell -ExecutionPolicy Bypass `
  -File research_pipeline/scripts/run-iterative.ps1 `
  -RunDir research_runs/tabular-foundation-multimodal-causal/20260331_run01 `
  -Mode experiment -AutoConfirm
...
bash research_pipeline/scripts/run-iterative.sh \
  --task research_pipeline/tasks/my-task.json \
  --mode full --auto-confirm

# 仅补跑实验
bash research_pipeline/scripts/run-iterative.sh \
  --task research_pipeline/tasks/my-task.json \
  --mode experiment --auto-confirm
...
# 指定运行目录（覆盖任务文件中的 iteration.run_dir）
bash research_pipeline/scripts/run-iterative.sh \
  --task research_pipeline/tasks/my-task.json \
  --run-dir research_runs/tabular-foundation-multimodal-causal/20260331_run01 \
  --mode full --auto-confirm
```

### 3.5 LaTeX 排版模式（.md → .tex）
...
```
/tex-writer-oneclick
Paper draft: research_runs/tabular-foundation-multimodal-causal/20260331_run01/11_paper_draft_v17.md
...
11. 11_paper_draft.md
12. 12_review_report.md
13. 13_revision_plan.md
14. 14_truthfulness_report.md
15. state.json
...
- 论文草稿自动创建新版本号（如 `11_paper_draft_v18.md`）
...
[INNOVATION_DESIGNER]  写 04 + 05（假设与可行性矩阵）
 ...
 └──────────────────────────────────────────────────┘
    │
    ▼
[WRITING_AGENT]   写 11_paper_draft.md
    │
    ▼
 ┌──────────────────────────────────────────────────┐
 │     【NEW】论文真实性检验门控                     │
 ...
 │       ↓ 所有宣称真实?                             │
 │       是 → 进入评审                               │
 │       否 → 产出 14_truthfulness_report.md         │
 │            → 回到 WRITING_AGENT 修正草稿          │
 │            （最多 truthfulness_patience 轮，默认2）│
 └──────────────────────────────────────────────────┘
    │
    ▼
[REVIEWER_AGENT]  写 12_review_report.md
    │
...
6. 论文草稿完成后，**【NEW】真实性检验**会自动交叉验证论文中的所有数据宣称和方法描述是否与代码实现、实验结果一致。若发现不一致，会产出 `14_truthfulness_report.md` 并要求 WRITING_AGENT 修正（最多 `truthfulness_patience` 轮）。
7. 论文真实性通过后进入评审。用 `12_review_report.md` 中的批评意见驱动修改反馈循环。
...
**工作流程**：
1. 逐项检查 `11_paper_draft.md` 中的宣称。
2. 若全部通过：在 `state.json` 中标记 `phase_status.truthfulness = passed`，进入评审。
3. 若发现不一致：
   - 产出 `14_truthfulness_report.md`，逐条列出：宣称原文、实际证据、判定结果（match / mismatch / unverifiable）。
...
在 `research_pipeline/tasks/my-task.json` 中添加修改循环参数：
...
每轮修改的输出文件按需更新（例如第二轮修改会覆盖 `11_paper_draft.md` 和 `12_review_report.md`）。
...
建议在任务文件（`research_pipeline/tasks/my-task.json`）中明确：
...
Pipeline 和每个 Agent 都内置了"永不停顿"保障，确保任何单点故障不会终止整个流程。
...
| 中途手工跳过某阶段 | 输入 `skip` → 自动写 stub 并继续 |
| 运行中断后续跑 | 加 `-Resume` 参数，已有产物的阶段自动跳过 |
...
| **【NEW】论文真实性检验不通过** | 产出 `14_truthfulness_report.md`，回到 WRITING_AGENT 修正；耐心耗尽后附带报告进入评审 |
...
**Q: pipeline 中途卡住不动了？**  
A: 若未加 `-AutoConfirm`，脚本会等待你按 Enter；若已加 `-AutoConfirm` 仍停住，通常是命令执行阻塞或网络限速，检查终端最后一条命令和 `blocker_log.jsonl`。
...
**【NEW】Q: 修改反馈循环怎么总是收不敛？**  
A: 检查 `12_review_report.md` 中评审意见的质量。若评审意见过于宽泛或相互矛盾，可：
...
A: 真实性检验门控会在评审前自动检测。发现不一致时会产出 `14_truthfulness_report.md` 并要求 WRITING_AGENT 修正。若持续不一致：
   1. 检查 `14_truthfulness_report.md` 中的具体不一致列表。
...
**Q: 上次运行中断，如何续跑？**  
A: 加 `-Resume` 参数，pipeline 会读取最新 run 目录下的 `state.json`，跳过已完成的阶段。
...
/tex-writer-oneclick
Paper draft: research_runs/<topic_slug>/<run_id>/11_paper_draft.md
Target venue: NeurIPS
Run directory: research_runs/<topic_slug>/<run_id>
...
| 文件 | 职责 | 平台 |
|---|---|---|
| research_pipeline/scripts/run-pipeline.ps1 | 主编排脚本（全流程） | Windows |
| research_pipeline/scripts/run-pipeline.sh | 主编排脚本（全流程） | Linux / macOS |
| research_pipeline/scripts/run-iterative.ps1 | 迭代模式编排脚本 | Windows |
| research_pipeline/scripts/run-iterative.sh | 迭代模式编排脚本 | Linux / macOS |
| research_pipeline/scripts/lib/pipeline-helpers.ps1 | 工具函数库 | Windows |
| research_pipeline/scripts/lib/pipeline-helpers.sh | 工具函数库 | Linux / macOS |
| research_pipeline/tasks/task.schema.json | 任务文件 Schema | 通用 |
| research_pipeline/tasks/example-task.json | 可直接运行的示例 | 通用 |
...
复制 `research_pipeline/tasks/example-task.json` 后按以下说明修改：
