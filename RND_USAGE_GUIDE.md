...
5. THEORETICAL_ANALYST：从原理到设计的理论分析（问题形式化、理论基础、设计推导、复杂度分析、理论-实验桥接）。
6. WRITING_AGENT：论文草稿生成。
7. REVIEWER_AGENT：审稿与修订计划。
8. TEX_WRITER：将 .md 论文草稿转换为符合学术会议要求的 .tex 论文（支持 AAAI / NeurIPS / ICML / ICLR / ACL 等）。
9. HUMANIZER：降低论文 AI 检测率，重写句式与结构，保留全部科学内容、数据和公式。
...
| `theory` | 更新理论分析：在实验结果变化后更新 `11_theoretical_analysis.md` |
| `paper` | 更新论文：使用最新实验结果和理论分析重写论文草稿 |
| `review` | 重新评审：对最新论文草稿重新生成评审意见与修订计划 |
| `revision` | 修订闭环：评审 → 修改假设 → 补跑实验 → 更新理论分析 → 重写论文 → 再评审，多轮迭代直至收敛 |
| `full` | 全量迭代：audit + experiment + theory + paper + review，一次性完成所有增量更新 |
...
[INNOVATION_DESIGNER]  写 04 + 05（假设与可行性矩阵）
 ...
 │  Part I: 问题形式化 + 理论基础 + 原理→设计映射  │
 │  Part II: 复杂度、收敛性、误差界、泛化分析     │
 │  Part III: 理论预测 vs 实验结果桥接             │
 └──────────────────────────────────────────────────┘
    │
    ▼
...
[REVIEWER_AGENT]  写 14_review_report.md
 ...
 │  R4: [THEORETICAL_ANALYST] 更新理论分析（原理→设计映射）  │
 │  R5: [WRITING_AGENT] 根据新结果+理论分析重写论文          │
 │  R6: [REVIEWER_AGENT] 重新审稿                            │
 │       ↓ 重大问题数量 < threshold?                        │
 │       是 → 退出循环    否 → 返回 R1（继续修改）           │
 └────────────────────────────────────────────────────────────┘
    │
...
3. 若满足预期：进入理论分析阶段（THEORETICAL_ANALYST）。
4. 若不满足预期：
   ...
   - 若仍不满足，在 `10_iteration_decisions.md` 中标注 `[EXPECTATION UNMET]`，带着最佳可用结果进入理论分析。
...
理论分析阶段在**结果预期检验通过之后、论文写作之前**自动执行。由专用的 `THEORETICAL_ANALYST` Agent 完成，产出 `11_theoretical_analysis.md`。

该阶段的核心目标是建立**从理论原理到方法设计的完整推导链**，确保方法的每个设计选择都有理论依据，并将理论预测与实验结果桥接。

### 产出结构

`11_theoretical_analysis.md` 包含三个部分：

**Part I — 从原理到设计（必选）**：
1. **问题形式化**：任务的精确数学表述、输入/输出空间、符号约定
2. **理论基础**：方法所依赖的核心理论原理（如信息论、变分推断、因果推断等），引用 SOTA 证据表中的来源
3. **原理→设计映射表**：每个设计选择对应的理论原理、推导理由、以及被放弃的备选方案
4. **方法形式化**：将提出的方法表述为完整的算法或优化问题

**Part II — 形式化分析**：
- 计算复杂度分析（必选）：时间/空间复杂度 + 与基线方法的复杂度对比表
- 收敛性分析（条件性）：如方法涉及迭代优化
- 误差界/近似保证（条件性）：如方法涉及近似、采样或松弛
- 泛化分析（条件性）：如方法涉及统计学习
- 可辨识性/一致性（条件性）：如方法涉及结构性声明

**Part III — 理论-实验桥接（必选）**：
- 每个理论结果的实验预测 vs `09_experiment_results.md` 中的实际结果
- 与已有理论框架的关系定位
- 理论分析的局限性和开放问题
...
- 每个定理/命题必须先列假设，再给出结论
- 区分严格证明与证明草案（标记 `[Proof Sketch]`）
- 不虚构定理；标记猜想为 `[Conjecture]`
- 每个理论结果后交叉引用实验验证

### 深度校准

- 若方法有强理论基础（优化、统计、因果）：产出完整分析
- 若方法主要是经验性的：只写 Part I（原理到设计）和必选的复杂度分析，标注 `theoretical_depth: minimal`
- 所有情况下 Part III（理论-实验桥接）均为必选
...
4. **R4 更新理论分析**：THEORETICAL_ANALYST 根据新实验结果刷新理论分析（原理→设计映射、理论-实验桥接）。
5. **R5 论文重写**：WRITING_AGENT 以新的实验数据和理论分析为基础重新组织论文内容，强调改进点。
6. **R6 再次评审**：REVIEWER_AGENT 对新论文进行独立评审。
7. **收敛判断**：
...
| **【NEW】结果预期检验不通过** | 记录差距到 `10_iteration_decisions.md`，回到创新+实验循环；耐心耗尽后标注 `[EXPECTATION UNMET]` 继续理论分析 |
| **理论分析 Agent 未产出或不完整** | 若无输出：自动写入 stub 的 `11_theoretical_analysis.md`（仅含问题定义和复杂度占位），标记 `theoretical_depth: minimal` 继续写作。若输出不完整（缺少 Part I 或 Part III）：标记 `phase_status.theoretical_analysis = incomplete`，在 `blocker_log.jsonl` 记录缺失部分，继续写作但在论文中标注 `[THEORY INCOMPLETE]` |
